...
- `store.stream("order", { events: [...] })` defines the write-time contract of allowed events; `store.aggregate(stream, AccountSchema, fold)` references a stream and defines read-time fold logic. Live folding is invoked as `store.fold(schema, streamId)`, keyed by the aggregate's output schema — the same schema-first shape as document reads — so several aggregates may fold the same stream independently; registering two aggregates with the same output schema is rejected as ambiguous.
